...
1. **Feature specification filed**:  The speclet should be filed as a GitHub issue and contain:
    ...
    a. A description of the feature (including any syntax changes involved)
    a. Discussions about impacted areas, such as overload resolution and type inference.
    a. Proposed changes to the API surface area.
...
1. **Prototype decision**: When the prototype phase ends, the Language Team will make a decision on the feature: 
    * **Ready to integrate**: The feature implementation revealed no blocking issues, has a healthy set of test suites, and is sufficiently complete in the compiler, IDE, debugger, etc. to integrate into the main tree on a selected branch.  
