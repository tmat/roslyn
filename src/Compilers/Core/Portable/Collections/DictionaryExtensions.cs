...
namespace Microsoft.CodeAnalysis
{
    ...
    internal static class DictionaryExtensions
    {
        ...
        public static void Add<K, V>(this IDictionary<K, ArrayBuilder<V>> dictionary, K key, V value, bool usePool)
            where K : notnull
        {
            if (!dictionary.TryGetValue(key, out var values))
            {
                values = usePool ? ArrayBuilder<V>.GetInstance() : new ArrayBuilder<V>();
