...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        private LocalSymbol _currentContextVariable;
            ...
            : base(previous)
        {
            ...
            _currentContextVariable = boundNodeFactory.SynthesizedLocal(instrumentationType, methodBody.Syntax, kind: SynthesizedLocalKind.LocalStoreTracker);
        }
            ...
            [NotNullWhen(true)] out LocalStateTrackingInstrumenter? instrumenter)
        {
            ...
            instrumenter = new LocalStateTrackingInstrumenter(instrumentationType, methodBody, boundNodeFactory, diagnostics, previous);
            return true;
        }
        ...
        public override void PreInstrumentBlock(BoundBlock original, LocalRewriter rewriter)
        {
            ...
            if (rewriter.CurrentLambdaBody == original)
            {
                ...
                _lazyPreviousContextVariables.Push(_currentContextVariable);
                _currentContextVariable = _factory.SynthesizedLocal(_instrumentationType, original.Syntax, kind: SynthesizedLocalKind.LocalStoreTracker);
            }
        }
        ...
        public override void InstrumentBlock(BoundBlock original, LocalRewriter rewriter, ref TemporaryArray<LocalSymbol> additionalLocals, out BoundStatement? prologue, out BoundStatement? epilogue, out BoundBlockInstrumentation? instrumentation)
        {
            ...
            foreach (var parameter in _factory.CurrentFunction.Parameters)
            {
                ...
                {
                    prologueBuilder.Add(_factory.ExpressionStatement(_factory.Call(receiver: _factory.Local(_currentContextVariable), parameterLogger, new[]
                    {
            ...
            var instrumentationPrologue = (entryLogger != null) ?
                _factory.Assignment(_factory.Local(_currentContextVariable), _factory.Call(receiver: null, entryLogger, entryArgs)) : _factory.NoOp(NoOpStatementFlavor.Default);
            ...
            var instrumentationEpilogue = (returnLogger != null) ?
                _factory.ExpressionStatement(_factory.Call(receiver: _factory.Local(_currentContextVariable), returnLogger)) : _factory.NoOp(NoOpStatementFlavor.Default);
        ...
        public override BoundExpression InstrumentUserDefinedLocalAssignment(BoundAssignmentOperator original)
        {
            ...
            {
                _factory.Call(
                    receiver: _factory.Local(_currentContextVariable),
                    logger,
