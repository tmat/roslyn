...
namespace Microsoft.CodeAnalysis.ExternalAccess.FSharp.Internal.Editor
{
    ...
    internal class FSharpEditorFormattingService : IFormattingInteractionService
    {
        ...
        async Task<ImmutableArray<TextChange>> IFormattingInteractionService.GetFormattingChangesAsync(Document document, TextSpan? textSpan, FormatterOptions? options, CancellationToken cancellationToken)
        {
        ...
        async Task<ImmutableArray<TextChange>> IFormattingInteractionService.GetFormattingChangesAsync(Document document, char typedChar, int position, FormatterOptions? options, CancellationToken cancellationToken)
        {
            var changes = await GetFormattingChangesAsync(document, typedChar, position, cancellationToken).ConfigureAwait(false);
            return changes?.ToImmutableArray() ?? ImmutableArray<TextChange>.Empty;
        }

        async Task<ImmutableArray<TextChange>> IFormattingInteractionService.GetFormattingChangesOnPasteAsync(Document document, TextSpan textSpan, FormatterOptions? options, CancellationToken cancellationToken)
        {
            var changes = await GetFormattingChangesOnPasteAsync(document, textSpan, cancellationToken).ConfigureAwait(false);
            return changes?.ToImmutableArray() ?? ImmutableArray<TextChange>.Empty;
        }

        async Task<ImmutableArray<TextChange>> IFormattingInteractionService.GetFormattingChangesOnReturnAsync(Document document, int position, FormatterOptions? options, CancellationToken cancellationToken)
        {
