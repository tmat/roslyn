...
";
        ...
        private static void GenerateSource(GeneratorExecutionContext context)
        {
            ...
            void Generate(string source, string fileName)
            {
                ...
                {
                    index += Marker.Length;
                    var closingComment = source.IndexOf("*/", index) + 1;
                    context.AddSource($"Generated_{fileName}", source[index..closingComment]);
                }
            }
        }
...
";
        ...
        [Fact]
        public async Task BreakMode_ValidSignificantChange_SourceGenerators_DesignTimeConditional()
        {
            var config = new[] { ("microsoft_codeanalysis_designtime", "true") };

            var sourceV1 = @"
class C { int Y => CompileTime.X + 1; } 
";
            var sourceV2 = @"
class C { int Y => CompileTime.X + 2; } 
";
            using var workspace = CreateWorkspace();
            ...
            var generator = new TestSourceGenerator()
            {
                ExecuteImpl = context =>
                {
                    var isDesignTime = context.AnalyzerConfigOptions.GlobalOptions.TryGetValue("microsoft_codeanalysis_designtime", out var value) && bool.Parse(value);

                    context.AddSource("Generated.cs",
                        "class " + (isDesignTime ? "DesignTime" : "CompileTime") + " { public static int X => 1; }");
                }
            };

            // microsoft_codeanalysis_designtime is setat design time:
            var project = AddDefaultTestProject(workspace, sourceV1, generator: generator, analyzerConfig: config);

            // microsoft_codeanalysis_designtime is not set at compile time
            LoadLibraryToDebuggee(EmitLibrary(sourceV1, generator: generator));
