...
namespace Microsoft.CodeAnalysis.EditAndContinue.UnitTests
{
    ...
    public sealed class EditAndContinueWorkspaceServiceTests : TestBase
    {
        ...
        private DebuggingSession StartDebuggingSession(EditAndContinueWorkspaceService service)
        {
        ...
        private void MarkAllDocumentsInSync(DebuggingSession session, Solution solution)
        {
        ...
        private SourceText CreateSourceTextFromFile(string path)
        {
            ...
            return SourceText.From(path, Encoding.UTF8, SourceHashAlgorithm.Sha256);
        }
...
{ 
  ...
  void M() 
  {
    ...
    System.Console.WriteLine(30); 
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task BreakMode_ValidSignificantChange_DocumentOutOfSync(bool userUpdatesDocumentBeforeFileWatcherDoes)
        {
            ...
            var document1 = project.AddDocument("test.cs", SourceText.From("class C1 { void M() { System.Console.WriteLine(0); } }"), filePath: sourceFile.Path);

            var (_, moduleId) = EmitAndLoadLibraryToDebuggee(sourceOnDisk, project.Id, sourceFile.Path);
            ...
            if (userUpdatesDocumentBeforeFileWatcherDoes)
            {
                // The user opens the source file and changes the source before Roslyn receives file watcher event.
                // EnC service listens to workspace changes and checks the file content. The file content matches the PDB,
                // so the document is now in-sync.
                workspace.ChangeDocument(document1.Id, SourceText.From("class C1 { void M() { System.Console.WriteLine(2); } }"));
                var document2 = workspace.CurrentSolution.Projects.Single().Documents.Single();
            ...
            workspace.ChangeDocument(document1.Id, GetSourceTextWithBinaryChecksum(sourceOnDisk));
