...
public unsafe class LocalStoreTrackerTests : IDisposable
{
    ...
    private string[] ReadRecords(BlobReader reader, Type[] localSignature, Type[] parameterSignature)
    {
        ...
        while (true)
        {
            ...
            var recordDisplay = recordKind switch
            {
                ...
                RecordKind.MethodEntry => $"{reader.ReadCompressedInteger():X}: Entered",
                RecordKind.LambdaEntry => $"{reader.ReadCompressedInteger():X}: Entered lambda {reader.ReadCompressedInteger():X}",
            ...
            string ReadUnmanaged(bool isLocal)
            {
                ...
                return $"{methodId:X}: {display} = {Inspect(value)}";
            }
            ...
            string ReadUntyped(bool isLocal)
            {
                ...
                return $"{methodId:X}: {display} = {Inspect(value.value)} ({Inspect(value.type)})";
            }
    ...
    public void Invariants()
    {
        ...
        Assert.Equal(LocalStoreTracker.LiftedVariableBaseIndex, Cci.MetadataWriter.LiftedVariableBaseIndex);
    ...
    [Theory]
    [CombinatorialData]
    public void LogLocalStoreEnums(bool isLocal)
    {
        var reader = GetReader();
    ...
    public void LogLocalStoreUnmanaged(bool isLocal)
    {
        ...
        var halfPi = Half.Pi;
        var nullableValue = (int?)123;
        var nullableNull = (int?)null;
        var valueTuple = (DateTime.Now, (int?)1, new Int128(1, 2));

        if (isLocal)
        {
            tracker.LogLocalStoreUnmanaged(ref halfPi, 0);
            tracker.LogLocalStoreUnmanaged(ref nullableValue, 1);
            tracker.LogLocalStoreUnmanaged(ref nullableNull, 2);
            tracker.LogLocalStoreUnmanaged(ref valueTuple, 3);
        }
        else
        {
            tracker.LogParameterStoreUnmanaged(ref halfPi, 0);
            tracker.LogParameterStoreUnmanaged(ref nullableValue, 1);
            tracker.LogParameterStoreUnmanaged(ref nullableNull, 2);
            tracker.LogParameterStoreUnmanaged(ref valueTuple, 3);
        }
    ...
    public void LogLocalStore_Lifted(bool isLocal)
    {
        ...
        if (isLocal)
        {
            ...
            // unmanaged:
            tracker.LogLocalStoreUnmanaged(ref closure.a__12, GetLiftedIndex(12));
            tracker.LogLocalStoreUnmanaged(ref closure.a__13, GetLiftedIndex(13));
            // TODO: generic type param
            // tracker.LogLocalStoreUnmanaged(ref closure.a__14, GetLiftedIndex(14));
        ...
        else
        {
            ...
            // unmanaged:
            tracker.LogParameterStoreUnmanaged(ref closure.a__12, GetLiftedIndex(12));
            tracker.LogParameterStoreUnmanaged(ref closure.a__13, GetLiftedIndex(13));
            // TODO: generic type param
            // tracker.LogLocalStoreUnmanaged(ref closure.a__14, GetLiftedIndex(14));
