    ...
    internal sealed class DebuggingSession : IDisposable
    {
            ...
            [NotNullWhen(true)] out ReaderWriterLockSlim? baselineAccessLock)
        {
            ...
            lock (_projectEmitBaselinesGuard)
            {
                ...
                baseline = new ProjectBaseline(project.Id, initialBaseline, generation: 0, localStateTrackerSourceTree: null);
        ...
        public async ValueTask<EmitInstrumentationUpdateResults> EmitInstrumentationUpdateAsync(Solution solution, ManagedHotReloadInstrumentation instrumentation, CancellationToken cancellationToken)
        {
            ThrowIfDisposed();

            var updateId = new UpdateId(Id, Interlocked.Increment(ref _updateOrdinal));

            var instrumentationUpdate = await EditSession.EmitInstrumentationUpdateAsync(solution, instrumentation, updateId, cancellationToken).ConfigureAwait(false);

            instrumentationUpdate.Log(EditAndContinueWorkspaceService.Log, updateId);
            _lastModuleUpdatesLog = instrumentationUpdate.ModuleUpdates.Updates;

            if (instrumentationUpdate.ModuleUpdates.Status == ModuleUpdateStatus.Ready)
            {
                Contract.ThrowIfNull(instrumentationUpdate.Results);
                StorePendingUpdate(new PendingInstrumentationUpdate(instrumentationUpdate.ProjectBaselines, instrumentationUpdate.ModuleUpdates.Updates));
            }

            return new EmitInstrumentationUpdateResults()
            {
                ModuleUpdates = instrumentationUpdate.ModuleUpdates,
                Diagnostics = instrumentationUpdate.Diagnostics,
            };            
        }
