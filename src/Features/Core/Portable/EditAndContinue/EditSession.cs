    ...
    internal sealed class EditSession : IDisposable
    {
            ...
            Project baseProject, Project project, CancellationToken cancellationToken)
        {
            ...
            foreach (var documentId in changes.GetChangedDocuments())
            {
                ...
                var oldDocument = await DebuggingSession.LastCommittedSolution.GetDocumentAsync(documentId, cancellationToken).ConfigureAwait(false);
                if (oldDocument == null)
                {
                    ...
                    outOfSyncDiagnostics.Add(Diagnostic.Create(descriptor, Location.None, new[] { oldDocument.Name }));
                    continue;
                }
